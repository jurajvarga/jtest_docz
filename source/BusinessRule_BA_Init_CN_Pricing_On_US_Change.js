...
{
  "id" : "BA_Init_CN_Pricing_On_US_Change",
  "type" : "BusinessAction",
  "setupGroups" : [ "BRG_ProductCreation" ],
  "name" : "BA_Init_CN_Pricing_On_US_Change",
  "description" : null,
  "scope" : "Global",
...
// BA_Init_CN_Pricing_On_US_Change
...
var revision = BL_Library.getRevisionsOfProductInWorkflow(product, "WF16_China_Maintenance_Workflow");
if (revision.length == 0) {
    ...
    if (ret[0] == "OK") {
    	   log.info('BA_Init_CN_Pricing_On_US_Change > Workflow > OK');
        ...
        newRegRevision.startWorkflowByID("Product_Maintenance_Workflow_Parent", "Initiated from " + newRegRevision.getValue("Workflow_Name_Initiated").getSimpleValue());
    	   ...
    	   log.info('BA_Init_CN_Pricing_On_US_Change > Workflow > Error');
        throw ret[2];
    }
} else {
    log.info('BA_Init_CN_Pricing_On_US_Change > Email');
    // ******** CST Logo ********
    ...
    emailBody += '<br><br> \
                  ...
                  but the China revision could not be initiated automatically \
                  as another China revision in process was already in progress.<br> \
                  Please access STEP to resolve this and start a new China revision for price change. \
                  <br><br>';
    ...
    if (recipientsEmails) {
        ...
        throw "BA_Init_CN_Pricing_On_US_Change: No recipients found.";
    }
}
}