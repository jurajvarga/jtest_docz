...
var isSentRevisionEU = conditionsRevision.valueOf(manager.getAttributeHome().getAttributeByID("US_to_EU_Email_Sent_YN")).eq("N");
...
var querySpecificationRevision = queryHomeRevision.queryFor(com.stibo.core.domain.Product).where(isSentRevisionEU.and((isProductRevision).or(isProductKitRevision).or(isEquipmentRevision)));
...
// ******** EU SKUs ******** 
emailBody += '<br>' + '<h2> SKU Regional EU Changes </h2>'
emailBody += '<table border="1" style="width:100%; border:1px solid black; border-collapse:collapse; background-color:#f1f1f1">';

var skuHeadingContent = ["SKU", "PRODUCT NO", "PRODUCT NAME", "STATUS", "PUBLISHED", "ITEM ACTIVE", "DE PREVIOUS PRICE", "DE CURRENT PRICE", "EU PREVIOUS PRICE", "EU CURRENT PRICE", "UK PREVIOUS PRICE", "UK CURRENT PRICE"];
...
var querySpecificationRevision = queryHomeRevision.queryFor(com.stibo.core.domain.Product).where(isSentRevisionEU.and(isRegionalRevision));
...
for (var i = 0; i < resultArrRegRevision.length; i++) {
    ...
    msRef.forEach(function(ref) {   //STEP-6396
        ...
        for (var j = 0; j < skus.size(); j++) {
		  ...
		  // STEP-6280 Rename SKU email flags for regional emails
            if (sku.getValue("EU_to_EU_Email_Sent_YN").getSimpleValue() != "N") { //STEP-6275 check for a new flag for regional changes on SKU
                continue;
            }
            ...
            var published = sku.getValue("PUBLISH_YN").getSimpleValue()
            var itemActive = sku.getValue("ITEMACTIVE_YN").getSimpleValue();
            var previousPriceDE = BL_Library.getPreviousPrice(sku, "DE");
            var currentPriceDE = sku.getValue("DE_CLP").getSimpleValue();
            var previousPriceEU = BL_Library.getPreviousPrice(sku, "EU");
            var currentPriceEU = sku.getValue("EU_CLP").getSimpleValue();
            var previousPriceUK = BL_Library.getPreviousPrice(sku, "UK");
            var currentPriceUK = sku.getValue("UK_CLP").getSimpleValue();

            if (!previousPriceDE || previousPriceDE == "") {
                previousPriceDE = currentPriceDE;
            }
            if (!previousPriceEU || previousPriceEU == "") {
                previousPriceEU = currentPriceEU;
            }
            if (!previousPriceUK || previousPriceUK == "") {
                previousPriceUK = currentPriceUK;
            }

            var revTableContent = [skuName, prodNo, prodName, prodStatus, published, itemActive, previousPriceDE,
                currentPriceDE, previousPriceEU, currentPriceEU, previousPriceUK, currentPriceUK
            ];
...
// change US_to_EU_Email_Sent_YN Flag for all revisions and it's corresponding SKUs and approve them
for (var i = 0; i < resultArrRevision.length; i++) {
    var revision = resultArrRevision[i];
    // STEP-6280 Rename SKU email flags for regional emails
    revision.getValue("US_to_EU_Email_Sent_YN").setSimpleValue("Y");
    ...
    msRef.forEach(function(ref) {    //STEP-6396
        ...
        for (var j = 0; j < skus.size(); j++) {
		  ...
		  // STEP-6280 Rename SKU email flags for regional emails
            if (sku.getValue("US_to_EU_Email_Sent_YN").getSimpleValue() == "N") {
                sku.getValue("US_to_EU_Email_Sent_YN").setSimpleValue("Y");
                log.info("changing US_to_EU_Email_Sent_YN on " + sku.getName() + " to " + sku.getValue("US_to_EU_Email_Sent_YN").getSimpleValue());
...
// change US_to_EU_Email_Sent_YN YN Flag for all regional revisions and corresponding SKUs
for (var i = 0; i < resultArrRegRevision.length; i++) {
    var regRevision = resultArrRegRevision[i];
    // STEP-6280 Rename SKU email flags for regional emails
    regRevision.getValue("US_to_EU_Email_Sent_YN").setSimpleValue("Y");
    ...
    msRef.forEach(function(ref) {    //STEP-6396
        ...
        for (var j = 0; j < skus.size(); j++) {
		  ...
		  // STEP-6280 Rename SKU email flags for regional emails
            if (sku.getValue("EU_to_EU_Email_Sent_YN").getSimpleValue() == "N") {
                sku.getValue("EU_to_EU_Email_Sent_YN").setSimpleValue("Y"); 
                log.info("changing EU_to_EU_Email_Sent_YN on " + sku.getName() + " to " + sku.getValue("EU_to_EU_Email_Sent_YN").getSimpleValue());
                baApprove.execute(sku);
            }
            // STEP-6275 ends
        }
...
// var recipientsEmails = "gabriel.pavlik@globallogic.com;maros.jevocin@globallogic.com;juraj.varga@globallogic.com";
...
if (recipientsEmails) {
    ...
    throw "BA_EUSkuEmail: No recipients found.";
}
// // STEP-5944 ends
}