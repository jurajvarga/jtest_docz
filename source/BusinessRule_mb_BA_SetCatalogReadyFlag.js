...
{
  "id" : "mb_BA_SetCatalogReadyFlag",
  "type" : "BusinessAction",
  "setupGroups" : [ "CatalogDiscountPrice" ],
  "name" : "mb_BA_SetCatalogReadyFlag",
...
// Round the price to two digits, if it is not an integer
var price = node.getValue("PRICE").getSimpleValue();
price = Number(price).toFixed(2);
price2 = Number(price).toFixed(0);
if ((node.getValue("PRICE").getSimpleValue() != price) && (node.getValue("PRICE").getSimpleValue() != price2)) {
    log.info("PRICE going to be updated to " + node.getValue("PRICE").getSimpleValue() + " to " + price);
	node.getValue("PRICE").setSimpleValue(price);
}
price = node.getValue("Future_Global_Base_Price").getSimpleValue();
price = Number(price).toFixed(2);
price2 = Number(price).toFixed(0);
if ((node.getValue("Future_Global_Base_Price").getSimpleValue() != price) && (node.getValue("Future_Global_Base_Price").getSimpleValue() != price2) ) {
	log.info("Future_Global_Base_Price going to be updated to " + node.getValue("Future_Global_Base_Price").getSimpleValue() + " to " + price);
