...
{
  "id" : "BA_CompleteOIEP_JSON_Formatter",
  "type" : "BusinessAction",
  "setupGroups" : [ "Outbound_Approved_Revision" ],
  "name" : "BA_CompleteOIEP_JSON_Formatter",
  "description" : null,
  "scope" : "Global",
...
*/
exports.operation0 = function (nodeHandlerSource,nodeHandlerResult,manager,executionReportLogger,BL_Email,BL_JSONCreation,BL_Library,BL_Maintenance,BL_ServerAPI) {
...
Object.buildNodeMessage = function buildNodeMessage(node, simpleEventType) {
    ...
    if (node.getObjectType().getID().equals("Service_Revision") || node.getObjectType().getID().equals("Service_Conjugates")) {
        mesg.eventid = "CurrentRevisionUpdated";
    }
    ...
    var prodCurRevnode = BL_Maintenance.getCurrentRevision(node) //STEP-6396
    ...
    if (prodCurRevnode) { //STEP-6396
        ...
        //Product Revision Figures -> Images with Application or  Protocol No is related to product  revision figures
                ...
                if (curRevKitCompChild.getObjectType().getID() == "Kit_Component") {
                    ...
                    if (licenseOnlyFlag == "N" && promotionOnlyFlag == "N") {
                        ...
                        if (kitRevSkuTargetNode != null) {
                            ...
                            //To Get Tech Transfer Lot for Kit Component
        ...
        for (var i = 0; i < asset.length; i++) {
            ...
            //var assetAppType = (assetJsonObj.Figure_Image_Application_Type === null || assetJsonObj.Figure_Image_Application_Type === 'null') ? true : false;
        ...
        var masterStockStepID = BL_JSONCreation.getProductRevision_To_MasterStock(manager, prodCurRevnode, notify); // STEP-6018
...
if(node != null) {
	if (simpleEventType == null) {
	    ...
	    executionReportLogger.logInfo("Event with ID '" + simpleEventType.getID() + "' passed to node handler for product # " + node.getValue("PRODUCTNO").getSimpleValue() + " product type " + node.getObjectType().getID() + " approvalStatus " + node.getApprovalStatus() );
	}
	...
	if (node.getApprovalStatus() == "Completely Approved") {
	    if (BL_Library.isProductType(node, checkServiceRevision) || BL_Library.isRevisionType(node, checkServiceRevision)) {
	        ...
	        if (nodeHandlerSource.isDeleted()) {
	            ...
	            if (BL_Library.isRevisionType(node, checkServiceRevision)) {
	            	BL_Email.notifyEmail(manager, node); //STEP-6363
	            } else { //STEP-6363 Added additional check for current revision
	            	var curRev = BL_Maintenance.getCurrentRevision(node);

	            	if(curRev) {
	            	    BL_Email.notifyEmail(manager, curRev); //STEP-6363
	            	}
	            }
	        }
