...
var isSentRevisionJapan = conditionsRevision.valueOf(manager.getAttributeHome().getAttributeByID("US_to_Japan_Email_Sent_YN")).eq("N");
...
var querySpecificationRevision = queryHomeRevision.queryFor(com.stibo.core.domain.Product).where(isSentRevisionJapan.and((isProductRevision).or(isProductKitRevision).or(isEquipmentRevision)));
...
for (var i = 0; i < resultArrRevision.length; i++) {
    ...
    msRef.forEach(function(ref) {    //STEP-6396
        ...
        for (var j = 0; j < skus.size(); j++) {
		  ...
		  // STEP-6280 Rename SKU email flags for regional emails
            if (sku.getValue("US_to_Japan_Email_Sent_YN").getSimpleValue() != "N") {
                continue;
            }
...
// ******** JAPAN SKUs ******** 
emailBody += '<br>' + '<h2> SKU Regional Japan Changes </h2>'
emailBody += '<table border="1" style="width:100%; border:1px solid black; border-collapse:collapse; background-color:#f1f1f1">';
...
var querySpecificationRevision = queryHomeRevision.queryFor(com.stibo.core.domain.Product).where(isSentRevisionJapan.and(isRegionalRevision));
...
for (var i = 0; i < resultArrRegRevision.length; i++) {
    ...
    msRef.forEach(function(ref) {    //STEP-6396
        ...
        for (var j = 0; j < skus.size(); j++) {
		  ...
		  // STEP-6280 Rename SKU email flags for regional emails
            if (sku.getValue("Japan_to_Japan_Email_Sent_YN").getSimpleValue() != "N") { //STEP-6275 check for a new flag for regional changes on SKU
                continue;
            }
            ...
            var published = sku.getValue("PUBLISH_YN").getSimpleValue()
            var itemActive = sku.getValue("ITEMACTIVE_YN").getSimpleValue();
            var previousPrice = BL_Library.getPreviousPrice(sku, "Japan");
            var currentPrice = sku.getValue("Japan_CLP").getSimpleValue();
            var region = "JP";
...
// change US_to_Japan_Email_Sent_YN Flag for all revisions and it's corresponding SKUs and approve them
for (var i = 0; i < resultArrRevision.length; i++) {
    var revision = resultArrRevision[i];
    // STEP-6280 Rename SKU email flags for regional emails
    revision.getValue("US_to_Japan_Email_Sent_YN").setSimpleValue("Y");
    ...
    msRef.forEach(function(ref) {  //STEP-6396
        ...
        for (var j = 0; j < skus.size(); j++) {
		  ...
		  // STEP-6280 Rename SKU email flags for regional emails
            if (sku.getValue("US_to_Japan_Email_Sent_YN").getSimpleValue() == "N") {
                sku.getValue("US_to_Japan_Email_Sent_YN").setSimpleValue("Y");
                log.info("changing US_to_Japan_Email_Sent_YN on " + sku.getName() + " to " + sku.getValue("US_to_Japan_Email_Sent_YN").getSimpleValue());
...
// change US_to_Japan_Email_Sent_YN YN Flag for all regional revisions and corresponding SKUs
for (var i = 0; i < resultArrRegRevision.length; i++) {
    var regRevision = resultArrRegRevision[i];
    // STEP-6280 Rename SKU email flags for regional emails
    regRevision.getValue("US_to_Japan_Email_Sent_YN").setSimpleValue("Y");
    ...
    msRef.forEach(function(ref) {    //STEP-6396
        ...
        for (var j = 0; j < skus.size(); j++) {
		  ...
		  // STEP-6280 Rename SKU email flags for regional emails
            if (sku.getValue("Japan_to_Japan_Email_Sent_YN").getSimpleValue() == "N") {
                sku.getValue("Japan_to_Japan_Email_Sent_YN").setSimpleValue("Y");
                log.info("changing Japan_to_Japan_Email_Sent_YN on " + sku.getName() + " to " + sku.getValue("Japan_to_Japan_Email_Sent_YN").getSimpleValue());
                baApprove.execute(sku);
            }
            // STEP-6275 ends
        }
...
// Recipients
var recipientsEmails = BL_Email.getEmailsFromGroup(manager, lookUp, "WorkflowEmailUserGroupMapTable", "Japan_Price")
...
if (recipientsEmails) {
    ...
    throw "BA_JapanSkuEmail: No recipients found.";
}
// // STEP-5944 ends
}